...
class Annotator:
    ...
    def box_mark(self, box, colors, a):
        im_gpu = torch.as_tensor(self.im).permute(2, 0, 1) .flip(0) .contiguous() / 255
        tmp = torch.ones([self.im.shape[0], self.im.shape[1]])
        mark = []
        for idx, b in enumerate(box):
            tmp = torch.zeros([self.im.shape[0], self.im.shape[1]])
            x1, y1, x2, y2 = int(b[0]), int(b[1]), int(b[2]), int(b[3])
            tmp[y1:y2, x1:x2] = 1
            mark.append(tmp.unsqueeze(0))
        mks = torch.concat(mark, axis=0)
        self.masks(mks, colors, im_gpu, a)
